...
public class ModelGolem<T extends GolemBase> extends EntityModel<T> {
   ...
   */
  @Override
  public void render(final T entityIn, final float limbSwing, final float limbSwingAmount, final float ageInTicks, final float netHeadYaw,
   ...
   */
  @Override
  public void setRotationAngles(final T entity, final float limbSwing, final float limbSwingAmount, final float ageInTicks, final float netHeadYaw,
   ...
   */
  @Override
  public void setLivingAnimations(final T entity, final float limbSwing, final float limbSwingAmount, final float partialTickTime) {
    int i = entity.getAttackTimer();

    if (i > 0) {
      this.golemRightArm.rotateAngleX = -2.0F + 1.5F * triangleWave((float) i - partialTickTime, 10.0F);
      this.golemLeftArm.rotateAngleX = -2.0F + 1.5F * triangleWave((float) i - partialTickTime, 10.0F);
    } else {
