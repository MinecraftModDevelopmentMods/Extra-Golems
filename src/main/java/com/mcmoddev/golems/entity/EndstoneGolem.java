...
public class EndstoneGolem extends GolemBase {
    ...
    if (this.world.isRemote && this.hasAmbientParticles) {
      for (int i = 0; i < 2; ++i) {
        this.world.addParticle(ParticleTypes.PORTAL, this.posX + (this.rand.nextDouble() - 0.5D) * (double) this.getWidth(),
            ...
            this.posZ + (this.rand.nextDouble() - 0.5D) * (double) this.getWidth(), (this.rand.nextDouble() - 0.5D) * 2.0D, -this.rand.nextDouble(),
  ...
  protected boolean teleportToEntity(final Entity entity) {
    Vec3d vec3d = new Vec3d(this.posX - entity.posX,
        this.getBoundingBox().minY + (double) (this.getHeight() / 2.0F) - entity.posY + (double) entity.getEyeHeight(), this.posZ - entity.posZ);
