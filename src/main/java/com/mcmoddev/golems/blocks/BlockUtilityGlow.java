...
public class BlockUtilityGlow extends BlockUtility {
	...
	protected boolean isLightGolem(GolemBase golem) {
		for (EntityAITaskEntry entry : golem.tasks.taskEntries) {
			if (entry.action instanceof EntityAIPlaceSingleBlock && ((EntityAIPlaceSingleBlock) entry.action).stateToPlace.getBlock() instanceof BlockUtilityGlow /*TODO: CLEANUP!*/) {
				return true;
			}
		}
		return false;
