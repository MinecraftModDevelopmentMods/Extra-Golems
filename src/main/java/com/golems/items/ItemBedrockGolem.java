...
public final class ItemBedrockGolem extends Item {
  ...
  public static void spawnParticles(final World world, final double x, final double y, final double z,
      final double motion) {
    spawnParticles(world, x, y, z, motion, EnumParticleTypes.SMOKE_LARGE, 60);
  }

  public static void spawnParticles(final World world, final double x, final double y, final double z,
      final double motion, final EnumParticleTypes type, final int num) {
    if (world.isRemote) {
      for (int i = num + world.rand.nextInt(Math.max(1, num / 2)); i > 0; --i) {
        world.spawnParticle(type, x + world.rand.nextDouble() - 0.5D, y + world.rand.nextDouble() - 0.5D,
            z + world.rand.nextDouble() - 0.5D, world.rand.nextDouble() * motion - motion * 0.5D,
            world.rand.nextDouble() * motion * 0.5D, world.rand.nextDouble() * motion - motion * 0.5D);
      }
    }
  }
  ...
  public void addInformation(final ItemStack par1ItemStack, final World world, final List<String> par3List,
    ...
    if (GuiScreen.isShiftKeyDown()) {
      ...
      final String lorePressShift = TextFormatting.GRAY + trans("tooltip.press") + " " + TextFormatting.YELLOW
          + trans("tooltip.shift").toUpperCase() + " " + TextFormatting.GRAY + trans("tooltip.for_more_details");
