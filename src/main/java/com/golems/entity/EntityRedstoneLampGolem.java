...
public final class EntityRedstoneLampGolem extends GolemMultiTextured {
	...
	public static final BiPredicate<GolemBase, IBlockState> LIT_PRED = 
			new BiPredicate<GolemBase, IBlockState>() {
		@Override
		public boolean test(GolemBase golem, IBlockState toReplace) {
			return golem.isProvidingLight();
		}
	};
	...
	public EntityRedstoneLampGolem(final World world) {
		...
		this.tasks.addTask(9, new EntityAIUtilityBlock(this, state, cfg.getInt(FREQUENCY), 
				cfg.getBoolean(ALLOW_SPECIAL), EntityAIUtilityBlock.getDefaultBiPred(state).and(LIT_PRED)));
	}
	...
	public int getBrightnessForRender(final float partialTicks) {
		return isProvidingLight() ? 15728880 : super.getBrightnessForRender(partialTicks);
	}

	@Override
	public float getBrightness(final float partialTicks) {
		return isProvidingLight() ? 1.0F : super.getBrightness(partialTicks);
	}
