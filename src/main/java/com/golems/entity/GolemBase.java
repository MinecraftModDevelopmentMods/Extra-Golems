...
public abstract class GolemBase extends EntityCreature implements IAnimals {
	...
	public boolean attackEntityAsMob(Entity entity) {
		...
		float currentAttack = (float) this.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE)
				.getAttributeValue();
	...
	protected boolean addDrop(List<WeightedItem> dropList, Item item, int meta, int min, int max,
			int percentChance) {
		return dropList.add(new WeightedItem(item, meta, min, max, percentChance));
	}

	/** Adds a Block to the list of golem drops **/
	protected boolean addDrop(List<WeightedItem> dropList, Block block, int meta, int min, int max,
			int percentChance) {
		return dropList
