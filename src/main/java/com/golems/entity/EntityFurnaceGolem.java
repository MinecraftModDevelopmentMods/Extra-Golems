...
public final class EntityFurnaceGolem extends GolemBase {
	...
	public void onLivingUpdate() {
		...
		if (this.worldObj.isRemote && rand.nextInt(24) == 0) {
			// particle effects
			final double pMotion = 0.03D;
			worldObj.spawnParticle(this.hasFuel() ? EnumParticleTypes.FLAME : EnumParticleTypes.SMOKE_NORMAL,
					this.posX + worldObj.rand.nextDouble() * 0.4D - 0.2D + this.motionX * 8,
					this.posY + worldObj.rand.nextDouble() * 0.5D + this.height / 2.0D,
					this.posZ + worldObj.rand.nextDouble() * 0.4D - 0.2D + this.motionZ * 8,
					worldObj.rand.nextDouble() * pMotion - pMotion * 0.5D, worldObj.rand.nextDouble() * pMotion * 0.75D,
					worldObj.rand.nextDouble() * pMotion - pMotion * 0.5D);
		}
	...
	public EnumActionResult applyPlayerInteraction(final EntityPlayer player, final Vec3d vec, 
    		@Nullable final ItemStack itemstack, final EnumHand hand) {
		// allow player to add fuel to the golem by clicking on them with a fuel item
		ItemStack stack = itemstack;
		int burnTime = getBurnAmount(stack) * (player.isSneaking() && stack != null ? stack.stackSize : 1);
		if (stack != null && burnTime > 0 && (getFuel() + burnTime) <= MAX_FUEL) {
			// add the fuel
			this.addFuel(burnTime);
			// reduce the itemstack
			if(stack.stackSize > 1 && !player.isSneaking()) {
				stack.stackSize--;
			} else {
				stack = stack.getItem().getContainerItem(stack);
			}
			// update the player's held item
			player.setHeldItem(hand, (stack != null && stack.stackSize > 0) ? stack : null);
			// add particles
		...
		if (stack != null && stack.getItem() == Items.WATER_BUCKET) {
			...
			ItemBedrockGolem.spawnParticles(this.worldObj, this.posX, this.posY + this.height / 2.0D, this.posZ, 0.1D,
...
//	}
	...
	class InertGoal extends EntityAIBase {
		...
		public void updateTask() {
			...
			// set looking down
			final double lookX = golem.getLookVec().xCoord;
			final double lookY = Math.toRadians(-15D);
			final double lookZ = golem.getLookVec().zCoord;
