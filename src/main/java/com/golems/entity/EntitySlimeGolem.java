...
public final class EntitySlimeGolem extends GolemBase {
  ...
  public boolean attackEntityAsMob(final Entity entity) {
    if (super.attackEntityAsMob(entity)) {
      ...
      // knocks back the entity it's attacking (if it's adult and not attacking a
      // slime)
  ...
  public void setDead() {
    ...
    if (!this.world.isRemote && !this.isChild() && getConfig(this).getBoolean(ALLOW_SPLITTING)) {
      ...
      // set location
      slime1.setLocationAndAngles(this.posX + rand.nextDouble() - 0.5D, this.posY, this.posZ + rand.nextDouble() - 0.5D,
          this.rotationYaw + rand.nextInt(20) - 10, 0);
      slime2.setLocationAndAngles(this.posX + rand.nextDouble() - 0.5D, this.posY, this.posZ + rand.nextDouble() - 0.5D,
          this.rotationYaw + rand.nextInt(20) - 10, 0);
      // spawn the entities
  ...
  public void notifyDataManagerChange(DataParameter<?> key) {
    ...
    if (this.isChild()) {
      ...
      this.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE)
          .setBaseValue(getConfig(this).getBaseAttack() * 0.6F);
