...
public final class EntitySlimeGolem extends GolemBase {
	...
	protected void damageEntity(final DamageSource source, final float amount) {
		if (!this.isEntityInvulnerable(source)) {
			...
			if (!this.isChild() && source.getSourceOfDamage() != null && getConfig(this).getBoolean(ALLOW_SPECIAL)) {
				knockbackTarget(source.getSourceOfDamage(), this.knockbackPower);
			}
		}
	}
	...
	public void setDead() {
		...
		if(!this.worldObj.isRemote && !this.isChild() && getConfig(this).getBoolean(ALLOW_SPLITTING)) {
			GolemBase slime1 = new EntitySlimeGolem(this.worldObj, true);
			GolemBase slime2 = new EntitySlimeGolem(this.worldObj, true);
			...
			slime2.setLocationAndAngles(this.posX + rand.nextDouble() - 0.5D, this.posY, 
					 this.posZ + rand.nextDouble() - 0.5D, this.rotationYaw + rand.nextInt(20) - 10, 0);
			// spawn the entities
			this.getEntityWorld().spawnEntityInWorld(slime1);
			this.getEntityWorld().spawnEntityInWorld(slime2);
		}
