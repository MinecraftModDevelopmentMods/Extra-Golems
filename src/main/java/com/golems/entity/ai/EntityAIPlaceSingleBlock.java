...
/**
 * Places a single IBlockState every {@code tickDelay} ticks with certain
 * conditions
 **/
@Deprecated
public class EntityAIPlaceSingleBlock extends EntityAIBase {
   ...
   * @param stateIn        the IBlockState that will be placed every
   *                       {@code interval} ticks
   * @param interval       ticks between placing block
   * @param canReplacePred a Predicate to determine if {@code stateIn} should
   *                       replace a certain IBlockState
   **/
  public EntityAIPlaceSingleBlock(final GolemBase golemIn, final IBlockState stateIn, final int interval,
      final boolean cfgAllows, final Predicate<IBlockState> canReplacePred) {
    this.setMutexBits(8);
    this.golem = golemIn;
   ...
   *
   * @param golemIn  the GolemBase to use
   * @param stateIn  the IBlockState that will be placed every {@code interval}
   *                 ticks
   * @param interval ticks between placing block
   **/
  public EntityAIPlaceSingleBlock(final GolemBase golemIn, final IBlockState stateIn, final int interval,
      boolean configAllows) {
    this(golemIn, stateIn, interval, configAllows,
        toReplace -> toReplace.getMaterial().equals(Material.AIR) && !toReplace.getBlock().equals(stateIn.getBlock()));
  }
