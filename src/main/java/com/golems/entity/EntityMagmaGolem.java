...
public final class EntityMagmaGolem extends GolemBase {
		...
		if (this.allowMelting && !this.isChild()) {
			...
			if (x == this.stillX && z == this.stillZ) {
				...
				if (++this.ticksStandingStill >= this.meltDelay
					...
					this.worldObj.setBlockState(below, replace, 3);
					this.ticksStandingStill = 0;
				}
			} else {
				this.ticksStandingStill = 0;
	...
	public void setDead() {
		...
		if(!this.worldObj.isRemote && !this.isChild() && getConfig(this).getBoolean(ALLOW_SPLITTING)) {
			GolemBase slime1 = new EntityMagmaGolem(this.worldObj, true);
			GolemBase slime2 = new EntityMagmaGolem(this.worldObj, true);
