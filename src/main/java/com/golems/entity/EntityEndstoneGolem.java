...
public class EntityEndstoneGolem extends GolemBase {
	...
	public void updateAITasks() {
		...
		if (this.getAITarget() != null) {
			...
			if (this.getAITarget().getDistanceSqToEntity(this) > 25.0D 
					&& (rand.nextInt(30) == 0 || this.getAITarget().getAITarget() == this)) {
	...
	public void onLivingUpdate() {
		if (this.worldObj.isRemote && this.hasAmbientParticles) {
			for (int i = 0; i < 2; ++i) {
	...
	public boolean attackEntityFrom(final DamageSource src, final float amnt) {
		...
		if (src instanceof EntityDamageSourceIndirect) {
			...
			if(src.getEntity() instanceof EntityLivingBase && this.teleportToEntity(src.getEntity())) {
				this.setRevengeTarget((EntityLivingBase) src.getEntity());
			...
			if (rand.nextInt(this.chanceToTeleportWhenHurt) == 0
					|| (this.getAITarget() == null && rand.nextBoolean())
	...
	protected boolean teleportToEntity(final Entity entity) {
		...
		final double d1 = this.posX + (this.rand.nextDouble() - 0.5D) * 8.0D - vec3d.xCoord * d0;
		final double d2 = this.posY + (double) (this.rand.nextInt(16) - 8) - vec3d.yCoord * d0;
		final double d3 = this.posZ + (this.rand.nextDouble() - 0.5D) * 8.0D - vec3d.zCoord * d0;
