...
public class BlockUtilityGlowWater extends BlockUtilityGlow {
  public BlockUtilityGlowWater(final Material m, final float defaultLight, final int tickRate,
      final IBlockState replaceWith) {
    ...
    this.setDefaultState(this.blockState.getBaseState().withProperty(BlockLiquid.LEVEL, 0)
        .withProperty(BlockUtilityGlow.LIGHT_LEVEL, light));
  }
  ...
  public boolean shouldSideBeRendered(IBlockState blockState, IBlockAccess blockAccess, BlockPos pos, EnumFacing side) {
    ...
    return !offset.isSideSolid(blockAccess, pos.offset(side, 1), side.getOpposite())
        && offset.getMaterial() != Material.WATER;
  }
