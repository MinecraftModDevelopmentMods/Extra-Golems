...
public final class Config {
  ...
  private static String[] villageGolemSpawnsDesert = new String[] { GolemNames.STRAW_GOLEM, GolemNames.SANDSTONE_GOLEM,
      GolemNames.SANDSTONE_GOLEM, GolemNames.SANDSTONE_GOLEM, GolemNames.REDSANDSTONE_GOLEM, GolemNames.BONE_GOLEM,
      GolemNames.TERRACOTTA_GOLEM, GolemNames.ENDSTONE_GOLEM };
  private static String[] villageGolemSpawnsPlains = new String[] { GolemNames.WOODEN_GOLEM, GolemNames.STRAW_GOLEM,
      GolemNames.MELON_GOLEM, GolemNames.MUSHROOM_GOLEM, GolemNames.LEAF_GOLEM, GolemNames.WOOL_GOLEM };
  ...
  private static void initGolemConfigSets(final Configuration config) {
    ...
    GolemLookup.addConfig(EntityDispenserGolem.class,
        new GolemConfigSet(config, "Dispenser Golem", 78.0D, 5.2F)
    ...
    GolemLookup.addConfig(EntityEndstoneGolem.class,
        new GolemConfigSet(config, "Endstone Golem", 50.0D, 8.0F)
    ...
    GolemLookup.addConfig(EntityGlowstoneGolem.class,
        new GolemConfigSet(config, "Glowstone Golem", 8.0D, 12.0F)
    ...
    GolemLookup.addConfig(EntityHardenedClayGolem.class,
        new GolemConfigSet(config, "Hardened Clay Golem", 22.0D, 4.0F));
    GolemLookup.addConfig(EntityIceGolem.class,
        new GolemConfigSet(config, "Ice Golem", 18.0D, 6.0F)
            .addKey(EntityIceGolem.ALLOW_SPECIAL, true, "Whether this golem can freeze water and cool lava nearby")
            .addKey(EntityIceGolem.CAN_USE_REGULAR_ICE, true,
    ...
    GolemLookup.addConfig(EntityMagmaGolem.class,
        new GolemConfigSet(config, "Magma Golem", 46.0D, 4.5F)
            .addKey(EntityMagmaGolem.ALLOW_SPLITTING, true,
                "When true, this golem will split into 2 mini-golems upon death")
            .addKey(EntityMagmaGolem.ALLOW_LAVA_SPECIAL, false, "Whether this golem can slowly melt cobblestone")
            .addKey(EntityMagmaGolem.MELT_DELAY, 240, 1, 24000,
    ...
    GolemLookup.addConfig(EntityMelonGolem.class,
        new GolemConfigSet(config, "Melon Golem", 18.0D, 1.5F)
            .addKey(EntityMelonGolem.ALLOW_HEALING, true,
                "Whether this golem can randomly heal (about every " + randomHealSec + " sec)")
    ...
    GolemLookup.addConfig(EntityMushroomGolem.class,
        new GolemConfigSet(config, "Mushroom Golem", 30.0D, 3.0F)
            .addKey(EntityMushroomGolem.ALLOW_HEALING, true,
                "Whether this golem can randomly heal at night (about every " + randomHealSec + " sec)")
            .addKey(EntityMushroomGolem.ALLOW_SPECIAL, true, "Whether this golem can plant mushrooms randomly").addKey(
                EntityMushroomGolem.FREQUENCY, 420, 1, 24000, "Average number of ticks between planting mushrooms"));
    ...
    GolemLookup.addConfig(EntityNetherWartGolem.class,
        new GolemConfigSet(config, "Nether Wart Golem", 22.0D, 1.5F)
            .addKey(EntityNetherWartGolem.ALLOW_HEALING, true,
                "Whether this golem can randomly heal at night (about every " + randomHealSec + " sec)")
            .addKey(EntityNetherWartGolem.ALLOW_SPECIAL, true, "Whether this golem can plant netherwart randomly")
            .addKey(EntityNetherWartGolem.FREQUENCY, 880, 1, 24000,
                "Average number of ticks between planting nether wart if enabled"));
    ...
    GolemLookup.addConfig(EntityRedSandstoneGolem.class,
        new GolemConfigSet(config, "Red Sandstone Golem", 15.0D, 4.0F));
    ...
    GolemLookup.addConfig(EntityRedstoneLampGolem.class,
        new GolemConfigSet(config, "Redstone Lamp Golem", 28.0F, 6.0F)
    ...
    GolemLookup.addConfig(EntitySeaLanternGolem.class,
        new GolemConfigSet(config, "Sea Lantern Golem", 24.0D, 6.0F)
    ...
    GolemLookup.addConfig(EntitySlimeGolem.class,
        new GolemConfigSet(config, "Slime Golem", 58.0D, 2.5F)
            .addKey(EntitySlimeGolem.ALLOW_SPLITTING, true,
                "When true, this golem will split into 2 mini-golems upon death")
            .addKey(EntitySlimeGolem.ALLOW_SPECIAL, true, "Whether this golem can apply extra knockback when attacking")
            .addKey(EntitySlimeGolem.KNOCKBACK, 1.9412F, 0.001F, 10.0F,
                "How powerful the Slime Golem knockback is (Higher Value = Further Knockback)"));
    GolemLookup.addConfig(EntitySpongeGolem.class,
        new GolemConfigSet(config, "Sponge Golem", 20.0D, 1.5F)
    ...
    GolemLookup.addConfig(EntityStainedGlassGolem.class,
        new GolemConfigSet(config, "Stained Glass Golem", 9.0D, 12.0F));
    GolemLookup.addConfig(EntityStrawGolem.class,
        new GolemConfigSet(config, "Straw Golem", 10.0D, 1.0F)
  ...
  private static void loadOther(final Configuration config) {
    ...
    villageGolemSpawnsDesert = config.getStringList("Desert Village Golem Spawns", CATEGORY_OTHER,
        villageGolemSpawnsDesert,
        "The following golems will appear in villages in Desert biomes. (Duplicate entries increase chances)");
    villageGolemSpawnsPlains = config.getStringList("Plains Village Golem Spawns", CATEGORY_OTHER,
        villageGolemSpawnsPlains,
    ...
    if (desertGolems.isEmpty()) {
      ...
      for (final String s : villageGolemSpawnsDesert) {
        ...
        if (entityEntry != null && (GolemBase.class).isAssignableFrom(entityEntry.getEntityClass())) {
          ...
          ExtraGolems.LOGGER.error("Tried to parse an unknown entity from the config! Skipping '" + s
              + "' in \"Desert Village Golem Spawns\"");
      }
    }
    ...
    if (plainsGolems.isEmpty()) {
      for (String s : villageGolemSpawnsPlains) {
        ...
        if (entityEntry != null && (GolemBase.class).isAssignableFrom(entityEntry.getEntityClass())) {
          ...
          ExtraGolems.LOGGER.error("Tried to parse an unknown entity from the config! Skipping '" + s
              + "' in \"Plains Village Golem Spawns\"");
      }
    }
