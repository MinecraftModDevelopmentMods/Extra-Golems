...
public class GuiGolemBook extends GuiScreen {
  ...
  protected static final ResourceLocation TEXTURE = new ResourceLocation(ExtraGolems.MODID,
      "textures/gui/info_book.png");
  protected static final ResourceLocation CONTENTS = new ResourceLocation(ExtraGolems.MODID,
      "textures/gui/info_book_contents.png");
  ...
  private static final ItemStack[] ingredientsSpell = new ItemStack[] { new ItemStack(Items.PAPER),
      new ItemStack(Items.FEATHER), new ItemStack(Items.REDSTONE),
  ...
  private static final ItemStack[] ingredientsHead = new ItemStack[] { new ItemStack(Blocks.PUMPKIN),
      new ItemStack(GolemItems.golemPaper) };
  ...
  private static final void initGolemBookEntries(World world) {
    ...
    Collections.sort(ALPHABETICAL,
        (GolemBookEntry g1, GolemBookEntry g2) -> g1.getGolemName().compareTo(g2.getGolemName()));
  }
  ...
  public void initGui() {
    ...
    this.buttonPreviousPage = this
        .addButton(new GuiGolemBook.NextPageButton(idPrevPage, arrowX + ARROW_WIDTH, arrowY, false));
    this.buttonNextPage = this
        .addButton(new GuiGolemBook.NextPageButton(idNextPage, arrowX + BOOK_WIDTH - ARROW_WIDTH * 2, arrowY, true));
    ...
    this.buttonBlockLeft = this
        .addButton(new GuiGolemBook.BlockButton(idBlockLeft, this, new Block[] {}, blockX, blockY, GOLEM_BLOCK_SCALE));
    blockX = (this.width / 2) + MARGIN;
    this.buttonBlockRight = this
        .addButton(new GuiGolemBook.BlockButton(idBlockRight, this, new Block[] {}, blockX, blockY, GOLEM_BLOCK_SCALE));
  ...
  private void drawPageAt(final int cornerX, final int cornerY, final int pageNum) {
    ...
    case 1:
      ...
      this.drawTexturedModalRect(startX, startY, this.isScrolling ? SCROLL_W + DEF_SEP : 0, CONTENTS_H + DEF_SEP,
          SCROLL_W, SCROLL_H);
    ...
    case 2:
      ...
      body = "\n\n\n\n" + I18n.format("golembook.recipe_spell.recipe", title, trans("item.paper.name"),
          trans("item.feather.name"), trans("item.dyePowder.black.name"), trans("item.redstone.name"));
      drawBasicPage(cornerX, cornerY, title, body);
    ...
    case 3:
      ...
      body = "\n\n\n\n" + TextFormatting.getTextWithoutFormattingCodes(
          trans("golembook.recipe_head.recipe", title, trans("item.golem_paper.name"), trans("tile.pumpkin.name")));
      drawBasicPage(cornerX, cornerY, title, body);
  ...
  private void draw2x2GridAt(final int startX, final int startY, final ItemStack[] ingredients,
  ...
  private boolean isMouseOverScroll(final int mouseX, final int mouseY) {
    ...
    return mouseX >= scrollStartX && mouseY >= scrollStartY && mouseX < scrollStartX + SCROLL_W
        && mouseY < scrollStartY + CONTENTS_H - SCROLL_H / 2;
  }
   ...
   * partialTicks) { this.hovered = mouseX >= this.x && mouseY >= this.y && mouseX
   * < this.x + this.width && mouseY < this.y + this.height; } }
   */
  protected static class BlockButton extends GuiButton {
  ...
  protected static class GolemEntryButton extends GuiButton {
    ...
    public GolemEntryButton(final int buttonId, final GuiGolemBook guiIn, final GolemBookEntry entryIn, final int x,
        final int y) {
    ...
    public void drawButton(Minecraft mc, int mouseX, int mouseY, float partialTicks) {
      if (this.visible) {
        ...
        this.drawTexturedModalRect(this.x, this.y, CONTENTS_W + DEF_SEP, this.hovered ? ENTRY_H + DEF_SEP : 0, ENTRY_W,
            ENTRY_H);
