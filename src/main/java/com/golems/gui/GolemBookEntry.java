...
public class GolemBookEntry {
  ...
  public GolemBookEntry(@Nonnull GolemBase golem) {
    ...
    try {
      ...
      this.IMAGE = Minecraft.getMinecraft().getResourceManager().getResource(new ResourceLocation(img))
          .getResourceLocation();
    } catch (IOException e) {
      // System.out.println("NO IMAGE FOUND FOR " +
      // EntityList.getEntityString(golem));
  ...
  private String makePage() {
    ...
    page.append("\n" + TextFormatting.GRAY + trans("entitytip.health") + ": " + TextFormatting.BLACK + this.HEALTH
        + TextFormatting.DARK_RED + " \u2764" + TextFormatting.BLACK);
