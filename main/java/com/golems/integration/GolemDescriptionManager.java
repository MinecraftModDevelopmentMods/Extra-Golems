...
/**
 * Base class to get in-game information about types of golems.
 * Currently used for Waila and The One Probe integration.
 * @author sky01
 **/
public abstract class GolemDescriptionManager
{
	...
	public List<String> getEntityDescription(GolemBase golem)
	{
		...
		if(showAttack)
		{
			...
			String sAttack = TextFormatting.GRAY + trans("attack") + " : " + TextFormatting.WHITE + attack;
			list.add(sAttack);
		}
		...
		{
			String sColor = TextFormatting.BLUE + trans("click_change_texture");
			list.add(sColor);
		}
		...
		{
			String sFire = TextFormatting.GOLD + trans("is_fireproof");
			list.add(sFire);
		}
		
		// add knockback resist to tip if possible
		if(this.showKnockbackResist && golem.getEntityAttribute(SharedMonsterAttributes.KNOCKBACK_RESISTANCE).getBaseValue() > 0.8999D)
		{
			String sResist = TextFormatting.GRAY + trans("knockback_resist");
			list.add(sResist);
		}
		...
		if(showSpecial)
		{
			...
			{
				String sLight = TextFormatting.YELLOW + trans("lights_area");
				list.add(sLight);
			}
			...
			{
				String sIndestructible = TextFormatting.WHITE + "" + TextFormatting.BOLD + trans("indestructible");
			...
			{
				String sPotion = TextFormatting.LIGHT_PURPLE + trans("grants_self_potion_effects");
				if(Config.BOOKSHELF.getBoolean(EntityBookshelfGolem.ALLOW_SPECIAL)) list.add(sPotion);
			}

			// add blinding effect to tip if possible
			if(golem.getClass() == EntityCoalGolem.class)
			{
				String sPotion = TextFormatting.GRAY + trans("blinds_creatures");
				if(Config.COAL.getBoolean(EntityCoalGolem.ALLOW_SPECIAL)) list.add(sPotion);
			}

			// add interaction to tip if possible
			if(golem.getClass() == EntityCraftingGolem.class)
			{
				String sCraft = TextFormatting.BLUE + trans("click_open_crafting");
				list.add(sCraft);
			}
			...
			{
				String sTeleport = TextFormatting.DARK_AQUA + trans("can_teleport");
				if(Config.ENDSTONE.getBoolean(EntityEndstoneGolem.ALLOW_SPECIAL)) list.add(sTeleport);
			}

			// add freezing to tip if possible
			if(golem.getClass() == EntityIceGolem.class)
			{
				String sFreeze = TextFormatting.AQUA + trans("freezes_blocks");
				if(Config.ICE.getBoolean(EntityIceGolem.ALLOW_SPECIAL)) list.add(sFreeze);
			}

			// add potion effects to tip if possible
			if(golem.getClass() == EntityLapisGolem.class)
			{
				String sPotion = TextFormatting.LIGHT_PURPLE + trans("attacks_use_potion_effects");
				if(Config.LAPIS.getBoolean(EntityLapisGolem.ALLOW_SPECIAL)) list.add(sPotion);
			}

			// add potion effects to tip if possible
			if(golem.getClass() == EntityLeafGolem.class)
			{
				String sPotion = TextFormatting.DARK_GREEN + trans("has_regen_1");
				if(Config.LEAF.getBoolean(EntityLeafGolem.ALLOW_SPECIAL)) list.add(sPotion);
			}

			// add fire to tip if possible
			if(golem.getClass() == EntityMagmaGolem.class)
			{
				String sLava = TextFormatting.RED + trans("slowly_melts", trans("tile.stonebrick.name"));
			...
			{
				String sPlant = TextFormatting.GREEN + trans("plants_flowers");
				if(Config.MELON.getBoolean(EntityMelonGolem.ALLOW_SPECIAL)) list.add(sPlant);
			}

			// add planting to tip if possible
			if(golem.getClass() == EntityMushroomGolem.class)
			{
				String sPlant = TextFormatting.DARK_GREEN + trans("plants_shrooms");
				if(Config.MUSHROOM.getBoolean(EntityMushroomGolem.ALLOW_SPECIAL)) list.add(sPlant);
			}

			// add netherbrick specials to tip if possible
			if(golem.getClass() == EntityNetherBrickGolem.class)
			{
				String sFire = TextFormatting.RED + trans("lights_mobs_on_fire");
				if(Config.NETHERBRICK.getBoolean(EntityNetherBrickGolem.ALLOW_FIRE_SPECIAL)) list.add(sFire);
			}

			// add planting to tip if possible
			if(golem.getClass() == EntityNetherWartGolem.class)
			{
				String sPlant = TextFormatting.RED + trans("plants_warts");
				if(Config.NETHERWART.getBoolean(EntityNetherWartGolem.ALLOW_SPECIAL)) list.add(sPlant);
			}

			// add redstone power to tip if possible
			if(golem.getClass() == EntityRedstoneGolem.class)
			{
				String sRed = TextFormatting.RED + trans("emits_redstone_signal");
				if(Config.REDSTONE.getBoolean(EntityRedstoneGolem.ALLOW_SPECIAL)) list.add(sRed);
			}

			// add knockback to tip if possible
			if(golem.getClass() == EntitySlimeGolem.class)
			{
				String sKnock = TextFormatting.GREEN + trans("has_knockback");
				if(Config.SLIME.getBoolean(EntitySlimeGolem.ALLOW_SPECIAL)) list.add(sKnock);
			}

			// add water-drying to tip if possible
			if(golem.getClass() == EntitySpongeGolem.class)
			{
				String sWater = TextFormatting.YELLOW + trans("absorbs_water");
				if(Config.SPONGE.getBoolean(EntitySpongeGolem.ALLOW_SPECIAL)) list.add(sWater);
			}

			// add boom to tip if possible
			if(golem.getClass() == EntityTNTGolem.class)
			{
				String sBoom = TextFormatting.RED + trans("explodes");
				if(Config.TNT.getBoolean(EntityTNTGolem.ALLOW_SPECIAL)) list.add(sBoom);
			}
		}
		return list;
	}
	...
	{
		return I18n.format("entitytip.".concat(s), strings);
	}
}
